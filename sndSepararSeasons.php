...
// Função para inserir ou atualizar dados em uma tabela específica
function upsertSeasonData($conn, $season, $tableName) {
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $insertSql = "INSERT INTO $tableName (name, season, kills, death, assistant, headshot, bombDefused, bombPlanted, teamKill, KDA)
                          VALUES ('$name', '".$row["season"]."', ".$row["kills"].", ".$row["death"].", ".$row["assistant"].", 
...
upsertSeasonData($conn, 'Season1', 'sndSeason1');

// Inserir ou atualizar dados da Season2 na tabela sndSeason2
upsertSeasonData($conn, 'Season2', 'sndSeason2');
